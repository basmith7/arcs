...
## 16. Loss forensics — hard's losses are variance, not a leak

Section 15 set the bar for the next strength idea: it must explain a loss the bot actually suffers.
Nobody had ever looked at the losses. This section is that look, and the answer is that at this
resolution **there is no leak to find**.

### The anomaly, and the method

At two players `hard` beats `standard` 63%-37% on a zero twin floor — so one game in three, a
strictly stronger bot loses to a strictly weaker one. How much of that is dice and card luck, and
how much is a systematic hole?

Two 320-game corpora, regenerated by arena index (seed 1, Board2Frontiers, indices 0-319,
`seatsForGame`/`seedForGame`, the stepBot loop with `runBots`' exact threading), each game captured
as its full log plus per-decision margins and per-chapter power/hand snapshots:

1. **hard vs standard** — the measurement corpus. Losses mix leak and luck.
2. **hard vs its own twin** — the variance control. Twin losses are coin-flips by construction, so
   any feature correlated with losing *there* is what losing looks like, not what causes it.

Sanity held exactly: the regenerated corpora came out 62.8% and 50.0%. A real leak must
discriminate hard's losses to standard **beyond** its losses to the twin.

### The result — 24 features, nothing survives the control

Per-game features from the log and snapshots, ranked by |d(vs std)| - |d(twin)| (Cohen's d,
lost-vs-won):

| feature | d (vs std) | d (twin) | delta | reading |
| --- | --- | --- | --- | --- |
| power gap, ch5 | -1.96 | -1.73 | 0.23 | "behind at the end" — a tautology of losing |
| ambitions won | -1.81 | -1.61 | 0.19 | near-tautology: ambitions are the power |
| surpasses | -0.26 | +0.10 | 0.16 | small, inside noise at n=119 losses |
| declares made | -1.42 | -1.33 | 0.08 | effect, not cause |
| *everything else* | | | <=0.06 | nothing |

The mechanism space the register has ever implicated is all here and all flat: pip waste (d 0.19
vs -0.15), unspent hand at chapter end (0.06 vs -0.02), trophies given through the p14 rule (0.00
vs -0.06), interception suffered, battles initiated and received, initiative seizes, lead/copy/pivot
mix, decision margins (d -0.69 in **both** corpora — close calls are what being behind feels like,
not why), and resource events. Several features discriminate twin losses *more* than losses to
standard (early power gaps, copies), and the loss trajectory swings late — mean gap +0.16 at
chapter 2, -2.89 at chapter 3, -9.05 at chapter 5 — which is variance compounding, not a hole being
dug early and paid for later.

**The counterfactual stage was designed to validate the top discriminator by swapping the implicated
decisions in lost games and measuring the flip rate. It had no target.** The kill switch built into
the plan fired: hard's losses to standard are statistically indistinguishable from its losses to a
copy of itself.

### What this settles, and what it does not

- **The 37% is the price of a stochastic game**, not a defect. Arcs deals hidden hands and rolls
  dice; a 26-point edge on a zero floor is what a real skill gap looks like *through* that noise,
  and the residual losses are the noise.
- **It retroactively explains the experiment graveyard.** Six evaluator ideas measured null or
  worse (sections 3j, 3k, 6, 9, 12, 15) because there was no systematic loss for them to fix. The
  register's two wins — the goal layer and the replies — dated from when such losses still existed.
- **Limitations, honestly:** 24 aggregate features cannot rule out decision-level blunders that
  leave aggregates unmoved; 3-player play (kingmaking, table dynamics) is unexplored; and "no leak
  at this resolution" is a statement about these features on 320 games, not a proof.

### Where strength work goes from here

Not through the evaluator, until someone finds a loss this analysis could not see. The productive
levers left are the ones that never depended on a leak: behavioural quality (a no-op discard priced
